...
In the created database data for air quality and weather for various cities were stored in diffrent Tables. By writinga simple SQL query I am able to connect to the database and select the data (both air quality and weather) for a desired city. The selected data is seaved in cvs file which is ready for further analysis.

The detailed decription of what information are present in the created file regarding both air quality and weather conditions can be found in the file: <variables_description.md>

The approach presented in steps 01-03, allowed me to create an easy to use method for creating a file with both weather and air pollution information for a desired city. In this project two selected cities in France were compared: Paris and Lyon. However, as the database contains as well information for onthe locations in Europe, it is easy to repeat this process for any other city.
...
The columns which presented relavant information for Time Series analysis were the columns contianing the air pollution and date information, i.e. : 'datetime', 'pm25', 'pm10', 'o3', 'no2', 'so2' and 'co'.

The process of data cleaning and processing included:

- converting dattime column into datetime type.
- missing values processing:
  - columns 'so2' and 'co' were deleted due to the high number of missing values
  - spearate missing values were replaced by the average values of each pollutant for the specified month and year.
  ...
  - for Lyon there was a period of 6 months missing in the second half of 2021. Fortunatelly these values, were presented in the other csv file at the air quality data source. Therefore, I imputed the missing values from the diffrent file.
- Outliers. In order to check for outliers I draw a boxplot graph for each pollutant. Baisng on that visual represantation of the data I could state that there were no outliers in data recoreded both in Paris and Lyon.

### Cleaning for Supervised ML and EDA

The aim of second stage of cleaning was to preapre the data for Supervised Machine Learnig and EDA. This process included:

- exchnaging columns with information about air quality for the clean ones from the previous stage of cleaning
- changing date type to datetime 
- dropping columns 'latitude', 'longitude', 'adress', 'datetimeEpoch', 'description' as they contain wither the same infromation or too various information, so they are not useful for machine learning model
- Missing values processing:
  - columns 'snow', 'snowdepth', 'solarradiation', and 'solarenergy' were dropped due to the high number of missing values.
  - 1 missing value for precipittation was filled with 0, as from other columns it was clear that it did not rain that day.
  - 'windgust' contained 1 missing value and it was dropped due to the high correlation with windspeed column
  - 1 missing value of pressure was filled with the value from the proceeding day as it preseted simialr weather conditions.
- 'Conditions' column needed encoding as it was the only categorical column. The column was encoded by creating a columns with 0/1 values for each weather condition.
- New columns contianing information about month and day of the week were created, as they might be useful for ML model. After that column with date information was dropped.
- The data was checked for cllinearity between columns. The correlation matrix was created and one by one columns with the correlation above 90% were dropped. This included: 'tempmax', 'tempmin' (high correlation to 'temp').
- Outliers. The boxblot for numerical columns were drawn. Basing on this vicual represantation I can conclude that all of the values are in the reasonable range and there were no outliers.
- Column with a target variable was created baisng on the recorded values of PM2.5 concentartion. This pollutant is often used as an air quality indicator as its concentartion is related to the concentration of other pollutants. Therefore, I created three clases basing on the values of PM2.5:
  - Class 0: day with good air quality, PM2.5 concentartion below 50 µg/m³.
  - Class 1: day with moderate air quality, PM2.5 concentartion between 50 and 100 µg/m³.
  - Class 2: day with bad air quality, PM concentartion above 100 µg/m³.
- After creating the target column, the columns with the concentartion of pollutants ('pm25', 'pm10', 'o3' and 'no2') were dropped, but this was done befor ML, as they were also used for EDA.
...
- Decomposition in order to see trend line.
- Train/test split in proportion 80/20 in order to evaluate chosen model.
- Implementation of model proposed by FbProphet liabry firstly for tarin data.
- Claculation of a model error (RMSE - Root Mean Square Devaition)
- Fitting the model on the whole available data.
- Forecasting pollutant concentartion for 2022.
...
The purpose of this part of the project was to create a supervised machine learning model, which would correctly predict the air quality class (Good, Moderate or Bad) baisng on the weather conditions during that day.
For the Supervised Machine Learnirng part 4 models were evaluated:

- Logistic Regression,
- Randome Forest Classifier,
- Balnced Random Forest Classifier, and
- pipline with models proposed by TPOT library.

The process of Machine Learning Analysis included:

- random train/test split of data in proportion 80/20.
- standarization of the data for Logistic Regression model.
- Features selection using Select From Model, Recursive Feauture Elimination and Recursive Feauture Elimination with Corss-Validation.
  - The results of feature selection for all models were similar. Thery proposed follwing features which were further used for model: 'temp', 'humidity', 'precip', 'windspeed', 'pressure', 'cloudcover', 'visibility', 'uvindex'.
- Hyperparameters Tunning using grid search and randomized search. Intrestingly the models were giving bvetter performance using default parameters that the ones proposed from hyperparameters tunning for Paris data, therefore default parameters were used for model evaluation for Paris. On the other hand, for Lyon the results for hyperparameters tunning were used.
- Evaluatins of the Logistic Regression, Random Forest Classifier and Balanced Ranodm Forest Classifier mdoels.
- TOPT implemenation:
  - Paris: TPOT proposed Stacking Estimator using Extra Trees Clasifier and GaussianNB
  - Lyon: TPOT proposed Stacking Estimator using Extra Trees Clasifier and XGBClassifier
- Comparison of obatined results.

For both cities also Usupervised Machine Learnirg models (KMeans, Agglomerative Clastering and  DBSCAN) were checked in order to verify if data can be properly classtered from the perspective of air pollution. However, the obtain results were not satisfying (Silhouette Coefficient balow 0.3). Thus this part of the project was not developed further.
...
Exploratory Data Analysis was didided into two parts:

- firstly, the hystorcial data on air pollution and weather in Paris and Lyon was analysed and compared
- secondly, the results of Time Series with the forecast for 2022 were analysed.

The appropriate graphs were prepared. They can be found in the Jupyter notebook as weel as on the Streamlit dashborad.

*Using : python, pandas, matplotlib, seaborn, plotly, folium*

## 08 - Streamlit Dashboard

The results of EDA were used in order to prepare a dashboard app using Streamlit libary.
...
The prepared dashborad/app was published using Heroku
