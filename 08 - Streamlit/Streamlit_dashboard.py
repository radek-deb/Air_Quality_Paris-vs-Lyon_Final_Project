...
#Selection of year
year_pie = st.sidebar.selectbox('Please select a year:',(2014,2015,2016,2017,2018,2019,2020,2021))

#Selection of month
month_sel = st.sidebar.selectbox('Please select a month:',('January','Febuary','March','April','May','June','July','August','September','October','November','December'))
month_name = {'January':1,'Febuary':2,'March':3,'April':4,'May':5,'June':6,'July':7,'August':8,'September':9,'October':10,'November':11,'December':12}
month_chart=month_name[month_sel]

#Selection of pollutant
poll = st.sidebar.radio('Please select a pollutant:',('PM2.5','PM10','Ozone','Nitorgen dioxide'))
...
table = data[columns][(data[columns]['year']==year_pie) &(data[columns]['month']==month_chart)].groupby(['city']).agg('mean').round()
...
#################SECTION2
#Models fit
fit_pol = lyon_2.loc[lyon_2.poll==pollutant][['ds','yhat_lower','yhat_upper','yhat','y','month','day_of_week']]
name_poll = {'pm25':'PM2.5', 'pm10':'PM10','o3':'ozone','no2': 'nitrogen dioxide'}
fit_por = paris_2.loc[paris_2.poll==pollutant][['ds','yhat_lower','yhat_upper','yhat','y']]

#Forecast for 2022
fit_2022l=lyon_2.loc[(lyon_2.poll==pollutant) & (lyon_2.year==2022)][['ds','yhat_lower','yhat_upper','yhat','y']]
fit_2022p=paris_2.loc[(paris_2.poll==pollutant) & (paris_2.year==2022)][['ds','yhat_lower','yhat_upper','yhat','y']]

#Trend line
fit_tredl = lyon_2.loc[lyon_2.poll==pollutant][['ds','trend_lower','trend_upper','trend']]
fit_tredp = paris_2.loc[paris_2.poll==pollutant][['ds','trend_lower','trend_upper','trend']]

#Yearly seasonality
fit_yearlyl = lyon_2.loc[(lyon_2.poll==pollutant)&(lyon_2.year==2016)][['ds','yearly','month']]
fit_yearlyp = paris_2.loc[(paris_2.poll==pollutant)&(paris_2.year==2016)][['ds','yearly','month']]
...
#Weekly seasonality
fit_weeklyl = lyon_2.loc[(lyon_2.poll==pollutant)&(lyon_2.year==2016)][['ds','weekly','day_of_week']]
fit_weeklyp = paris_2.loc[(paris_2.poll==pollutant)&(paris_2.year==2016)][['ds','weekly','day_of_week']]
...
fig_pie.update_layout(
    ...
    annotations=[dict(text='PARIS', x=0.17, y=0.5, font_size=20, showarrow=False),
...
name_poll = {'pm25':'PM2.5', 'pm10':'PM10','o3':'ozone','no2': 'nitrogen dioxide'}
# Layout
fig_pollutant_year.update_layout(barmode='group', xaxis_tickangle=-45,title=f"Yearly concentration of {name_poll[pollutant]} in {year_pie}",xaxis_title="Date",
...
fig_metrics_lyon.update_layout(title=f'Average values for Lyon in {month_sel} {year_pie}',
    grid={"rows": 2, "columns": 3, "pattern": "independent"},
    template={"data": {"indicator": [{"mode": "number",}]}},
)
...
fig_bar_month.add_trace(
	go.Bar(x=['PM2.5','PM10','Ozone', 'Nitrogen dioxide'], y=table_pollutant['Lyon'], name='Lyon',marker_color='#C70039'),
)
fig_bar_month.add_trace(
...
name_poll = {'pm25':'PM2.5', 'pm10':'PM10','o3':'ozone','no2': 'nitrogen dioxide'}
# Layout
fig_bar_month.update_layout(barmode='group', xaxis_tickangle=-45,title=f"Concentration of analysed pollutants in {month_dict[month_chart]} {year_pie}",title_x=0.5,xaxis_title="Pollutant",
    yaxis_title="Concentartion",)
...
name_poll = {'pm25':'PM2.5', 'pm10':'PM10','o3':'ozone','no2': 'nitrogen dioxide'}
# Layout
fig_season_yearly.update_layout(barmode='group', xaxis_tickangle=-45,title=f"Yearly seasonality of {name_poll[pollutant]}",xaxis_title="Day of year",
    yaxis_title="Yearly",)
...
name_poll = {'pm25':'PM2.5', 'pm10':'PM10','o3':'ozone','no2': 'nitrogen dioxide'}
# Layout
fig_season_weekly.update_layout(barmode='group', xaxis_tickangle=-45,title=f"Weekly seasonality of {name_poll[pollutant]}",xaxis_title="Day of week",
    yaxis_title="Weekly",)
...
st.markdown("Welcome to this page. This is the final project I realized during the Ironhack Data Analytics Bootcamp.")
st.markdown("The goal of the project was to investigate and compare air pollution in Paris and Lyon and to forecast future air pollution using Time Series analysis and Supervised Machine Learning based on the weater data.")
st.markdown("The source code for this poroject can be found here: <https://github.com/radek-deb/Final_Project_Ironhack>")
